...
refs <- read_games(session) |>
    ...
    filter(ID != "") |>
    mutate_at(24:43, \(v) ifelse(is.na(v), 0, v)) |>  # Write zeros into all the NAs introduced in the join.
...
filter(refs, GamesIn22 > 0) |>
    arrange(desc(GamesIn22)) |>
    mutate(Rank = str_c(Rank.Center, '/', Rank.AR)) |>
    select(Games = GamesIn22,
           FullName,
           Age,
           Gender,
           City,
