...
refs <- read_games(session) |>
    ...
    summarize(
        l7v7 = sum(Game == '7v7'),
        l9v9 = sum(Game == '9v9'),
        l11v11 = sum(Game == '11v11'),
        Center = sum(Position == 'Center'),
        AR = sum(Position == "AR1" | Position == "AR2"),
        Fall = sum(Season == 'Fall'),
        Spring = sum(Season == 'Spring'),
        GamesIn21 = sum(Year == 2021),
        GamesIn20 = sum(Year == 2020),
        GamesIn19 = sum(Year == 2019),
        GamesIn18 = sum(Year == 2018), 
        GamesIn17 = sum(Year == 2017),
        Rank1 = sum(Rank == 1),
        Rank2 = sum(Rank == 2),
        Rank3 = sum(Rank == 3),
    ...
    left_join(x=read_refs(session), by='UID') |>  # read_refs returns refs currently in the org
    filter(ID != "") |>
    mutate_at(23:41, \(v) ifelse(is.na(v), 0, v)) |>  # Write zeros into all the NAs introduced in the join.
    mutate(Current = Registered.2022,
           Recent = GamesIn21 > 4,
           New = InitialYear >= 2021 & Total == 0,
           Inexperienced = Age < 16 | Total <= 8 & (l9v9 + l11v11) == 0)
...
n.current.active2021 <- sum(refs$Current & refs$GamesIn21 > 0)
n.active2021 <- sum(refs$GamesIn21)
```
...
margins <- filter(refs, LastYear >= 2017) |>
    ...
    mutate(Year = str_match(name, "GamesIn(..)")[,2] |> as.integer() + 2000) |>
    select(-name)

group_by(margins, Gender, Year) |>
    summarize(Games = sum(Games)) |>
    ggplot() +
        aes(fill=Gender, x=Year, y=Games) +
    geom_col() +
...
arrange(refs, desc(Total)) |>
    head(50) |>
    select(Name = FullName, Age, `Total Games`=Total, `11v11`=l11v11, `9v9`=l9v9, `7v7`=l7v7, `Reg 22?`=Registered.2022, 
